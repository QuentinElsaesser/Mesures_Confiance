...
class PlotBar():
    def __init__(self, name, metric_name, index_m, spe, directory="../png/", ranged=False):
        ...
        if self.index_m != self.spe:
            ...
            nb_val = len(constants.PLOT_ORDER)
            start = 0
    ...
    def myplot(self, ax, x, y, i, j, d=0, t=-1, mini=None, maxi=None, items=0):
        ...
        if self.condition(ind_method=j, i=i):
            if d == 0 and t < 0:
                #default
                if ax != None:
                    ax.bar(self.placement[self.curr_placement], y[i][j], self.width_bar, color=self.parameters_plot(j,1,i), label=self.parameters_plot(j,2,i))
                else:
                    plt.bar(self.placement[self.curr_placement], y[i][j], self.width_bar, color=self.parameters_plot(j,1,i), label=self.parameters_plot(j,2,i))
                ...
                if ax != None:
                    ax.bar(self.placement[self.curr_placement], y[i][j][d:], self.width_bar, color=self.parameters_plot(j,1,i), label=self.parameters_plot(j,2,i))
                else:
                    plt.bar(self.placement[self.curr_placement], y[i][j][d:], self.width_bar, color=self.parameters_plot(j,1,i), label=self.parameters_plot(j,2,i))
                self.curr_placement += 1
                ...
                if ax != None:
                    ax.bar(self.placement[self.curr_placement], y[i][j][d:t], self.width_bar, color=self.parameters_plot(j,1,i), label=self.parameters_plot(j,2,i))
                else:
                    plt.bar(self.placement[self.curr_placement], y[i][j][d:t], self.width_bar, color=self.parameters_plot(j,1,i), label=self.parameters_plot(j,2,i))
                self.curr_placement += 1
    ...
    def plot_one(self, i, d=0, t=-1, height=20, width=10):
        ...
        #     ax.set_xticks(self.xplacement, [int(n) for n in self.percent])
