...
class Latex:
    def __init__(self, nb_metrics, spe, path_f="../results/", nvalue="PRC", fixed_prc="", infograph="", readname="", distrib="", literals=""):
        """
        ...
        self.nb_literals = literals
        
        self.nvalue = nvalue
        self.fixed_prc = ""
        if nvalue == constants.RUN_VRN or nvalue == constants.RUN_SPR or nvalue == constants.RUN_SDR or nvalue == constants.RUN_BSA or nvalue == constants.RUN_BFA or nvalue == constants.RUN_SRC or nvalue == constants.RUN_FCT:
        ...
        if readname == "":
            ...
            f = open(name, "w")
            print(f"Create file {name} from {constants.PATH_XP}{readname}")
            f.close()
        self.name = name
    ...
    def new_subsection(self, prc, nb_exp, metric, fixed_prc):
        ...
        for i in range(self.nb_metrics):
            ...
            if i != self.spe:
                run_test = constants.METRICS_NAMES.index(metric.metrics_name[i]) in list(constants.ID_METRICS_ACTIVE.values())
                # self.section[i] += f"{prc}\% with {nb_exp} experiences for metric {metric.metrics_name[i]}.\n"
                self.section[i] += f"{self.texte(prc, fixed_prc, True)} with {nb_exp} experiences for metric {metric.metrics_name[i]} (run:{run_test}).\n"
                self.section[i] += "\n"
