...
class GenBelSynt:
    """
    refait toutes les sources de 0 avec les memes formules
    """
    ...
    def claims_src(self, para, ind_para):
        ...
        for i in range(self.nbs):
            ...
            if para[3]:
                if len(xxxx2) > 0 and i < para[5]:
                    self.croyances.append(xxxx2[random.randint(0, len(xxxx2)-1)])
                else:
    ...
    #     type_generation = ""
    #     para = [0,2,5,False,2,0]
    #     runit = 0
    #     it = 1000
    #     total = 0
        
    #     find_intv_src_save = [False for n in self.interval]
    ...
    #                 print(runit, "\n", res)
    #                 # ask nb it
    #                 tmp = input(f"{datetime.now().strftime('%H:%M:%S')} - (curr src {self.nbs}) nb it (curr {it}) ? ")
    #                 if tmp != "":
    #                     if tmp.isnumeric():
    #                         it = int(tmp)
    #                     else:
    ...
    #             num_intv = 0
    #         # for num_intv in range(len(self.interval)):
    ...
    #             self.claims = []
    #             self.croyances = []
    #             self.id_formulas = self.all_para[indice_para][3]
    #             self.formulas_id = self.all_para[indice_para][4]
    #             self.took = self.all_para[indice_para][5]
    #             self.truth = self.all_para[indice_para][6]
    #             error = self.claims_src(para, indice_para)
    #             if error != False:
    #                 if self.typeg == constants.RUN_BIF:
    #                     self.generate_votes_implication_full()
    #                 elif self.typeg == constants.RUN_BSA:
...
if __name__ == "__main__":
    ...
    # NB SOURCES
    typeg = "BSA"; nombre_src = [10,20,50,100]; intv = ['60-65']; int_variance = nombre_src 
