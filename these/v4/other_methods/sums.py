...
class Sums:
    ...
    def trust_sources(self):
        ...
        for i in range(len(self.G.trust_s)):
            # print(f"s{i} - {self.G.sf[i]} * {self.mem_fact} = {sum(self.G.sf[i]*self.mem_fact)}")
    ...
    def trust_fact(self):
        ...
        for i in range(len(self.G.trust_f)):
            # print(f"f{i} - {self.G.mat_fs[i]} * {self.G.trust_s} = {sum(self.G.mat_fs[i]*self.G.trust_s)}")
    ...
    def run(self):
        ...
        while not self.convergence():
            ...
            self.mem_fact = deepcopy(self.G.trust_f)
            
            self.trust_sources()
            maxi = max(self.G.trust_s)
            for i in range(len(self.G.trust_s)):
                #print(i, self.G.trust_s[i], maxi)
                self.G.trust_s[i] /= maxi
            
            self.trust_fact()
            maxi = max(self.G.trust_f)
            for i in range(len(self.G.trust_f)):
                #print(i, self.G.trust_f[i], maxi)
