...
class McIntersect(basemaxcons.BaseMaxCons):
    ...
    def decision(self):
        ...
        evalue la noombre de formule similaire entre les deux 
        la reponse est la plus haute valeur
        """
        # print("--------------------")
        self.answers = []
        ...
        #         a = self.agents[i]
        #         dist += len(set([str(n) for n in a]).intersection(set([str(p) for p in m])))
        #         # print("A", i, a)
        #         # print("M", m)
        #         # print(dist)
        #         # print()
        ...
        for i in range(len(distance)):
            # print("INTERS", distance[i], self.maxcons[i], len(self.maxcons[i]))
            if distance[i] == maxi:
