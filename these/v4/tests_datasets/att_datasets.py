...
class AttDatasets():
    def __init__(self, G, truth_obj, truth_fct):
        """
    ...
    def __str__(self):
        ...
        res += f"Avg Diff : {self.diff}"
        res += "\n"

        return res
    
    def compute_diff(self):
        nbfct = len(self.truth_fct)
        for i in range(len(self.G.sf)):
            s = self.G.sf[i]
            cpt = 0
            for j in range(len(s)):
                if s[j] == 1:
                    if s[j] != self.truth_fct[j]:
                        cpt += 1
            self.proba_src.append(cpt/nbfct)

        for i in range(len(self.proba_src)):
            self.diff_src.append(abs(self.proba_src[i] - self.G.trust_s[i]))
